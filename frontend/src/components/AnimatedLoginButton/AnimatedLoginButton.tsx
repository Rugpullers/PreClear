...
import './AnimatedLoginButton.css';
...
const AnimatedLoginButton: React.FC<AnimatedLoginButtonProps> = ({ onClick, isLoggedIn = false }) => {
    ...
    useEffect(() => {
        if (buttonRef.current && logoutButtonStates[state]) {
            const properties = logoutButtonStates[state];
            for (const key in properties) {
                buttonRef.current.style.setProperty(key, properties[key]);
            }
        }
    }, [state]);
    ...
    return (
        <button
            ref={buttonRef}
            className={`logoutButton ${isClicked ? 'clicked' : ''} ${isDoorSlammed ? 'door-slammed' : ''}`}
            onClick={handleClick}
        ...
        >
            ...
            <span className="button-text">{isLoggedIn ? 'Logout' : 'Login'}</span>
        </button>
    );
};
