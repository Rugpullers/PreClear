...
from backend.db.connection import predictions_collection
...
async def get_optimal_route(source: str, destination: str) -> Dict[str, Any]:
    """
    Calculates the fastest route using A* based on XGBoost prediction integration.
    """
    if source not in ROUTING_GRAPH or destination not in ROUTING_GRAPH:
        raise ValueError(f"Invalid source or destination node. Available nodes: {list(ROUTING_GRAPH.nodes)}")

    # 1. Fetch the latest predicted traffic states from MongoDB
    # Since nodes in graph = location_id in DB, we fetch predictions for all nodes.
    traffic_penalties = {}
    ...
    # Edge weight = base_weight * max(penalty of node u, penalty of node v)
    ...
    try:
        path = nx.astar_path(G, source, destination, heuristic=haversine, weight='current_time_weight')
        ...
        for i in range(len(path) - 1):
            ...
            route_details.append({
                ...
                "congestion_impact": "HIGH" if max(traffic_penalties[u], traffic_penalties[v]) == 3.0 else ("MEDIUM" if max(traffic_penalties[u], traffic_penalties[v]) == 1.5 else "LOW")
            })
