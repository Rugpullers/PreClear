...
# Cache for heatmap data (TTL 3 seconds for live updates)
heatmap_cache = TTLCache(maxsize=100, ttl=3)
...
async def get_traffic_heatmap() -> List[TrafficHeatmapResponse]:
    ...
    pipeline = [
        {"$sort": {"timestamp": -1}},
        {
            "$group": {
                "_id": "$junction_id",
                "total_vehicles": {"$first": "$vehicle_count"},
                "avg_vehicles": {"$first": "$vehicle_count"},
                "record_count": {"$sum": 1}
            }
        }
    ]
